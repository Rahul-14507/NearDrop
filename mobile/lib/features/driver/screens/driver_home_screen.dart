...
import 'package:neardrop/features/driver/widgets/batch_assignment_sheet.dart';
...
class _DriverHomeScreenState extends State<DriverHomeScreen> {
  ...
  void _showBatchAssignedSheet(Map<String, dynamic> event) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => BatchAssignmentSheet(
        batchCode: batchCode,
        ...
        onAccepted: () {
          ...
          if (deliveries.isNotEmpty) {
            ...
            final name =
                first['recipient_name'] as String? ?? 'the customer';
            final address =
                first['address'] as String? ?? 'your first stop';
            AzureTtsService()
                .speak('Batch accepted. Starting with $name at $address.');
          }
        },
        onRejected: () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Batch rejected')),
          );
        },
      ),
    );
